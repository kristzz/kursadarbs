...
interface BusinessMessagingProps {
  applicantId?: number; // Optional applicant ID to start conversation with
  businessId: number; // Current business ID
  token: string;
}

const BusinessMessaging: React.FC<BusinessMessagingProps> = ({ applicantId, businessId, token }) => {
  ...
  useEffect(() => {
    ...
    if (token) {
      websocketService.connect(`business-${businessId}`, token);
  ...
  // If applicantId is provided, start a conversation with that applicant
  useEffect(() => {
    if (applicantId) {
      startConversationWithApplicant(applicantId);
    }
  }, [applicantId]);
  ...
  const startConversationWithApplicant = async (applicantId: number) => {
    try {
      setLoading(true);
      const response = await api.post('/conversations', { user_id: applicantId });
      if (response.data.status) {
